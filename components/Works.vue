        ...
        <div class="flex flex-col space-y-6">
            ...
            <div>
                <div :class="`grid grid-cols-3`">
                    <span v-for="(tab, index) in tabs" :key="index" class="py-4 hover:text-white text-center text-xs sm:text-base border-b-2 cursor-pointer transition duration-200 hover:border-white" @click.prevent="selectTab(tab)" :class="selectedTab && selectedTab.index === tab.index ? 'text-white border-white' : 'text-gray-600 border-gray-600'">
                        {{ tab.title }}
                    </span>
                </div>
            </div>
